...
class Course:
    ...
    def addCourseDataFromFile(self,filename):
        ...
        for line in lines:
            line=line.strip('\n')
            if index==0:
                self.__courseID=line
            elif index==1:
                self.__courseName=line
            elif index==2:
                self.__semester=line       
            elif index==3:
                self.__instractor=line    
            elif index==4:
                tas=line.split(",")
                for ta in tas:
                  self.__courseTAs.append(ta.strip())  
            else:
                student_id,student_name,*assignments=line.split(':')
                marks=[eval(mark.strip()) for mark in assignments] 
                student=Student("","")
